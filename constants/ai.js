...
function castSpell(state, hero, opponent, cooldowns, settings) {
    ...
    if (candidates.length > 0) {
        ...
        var action = {
            spellId,
            target: jitter(opponent.pos, MissRadius)
        };
...
function validAttack(state, hero, opponent, spell) {
    ...
    if (spell.action === "spray") {
        if (spell.id === "a.w" && hero.health > 50) { // Swap doesn't work as an attack
            return false;
        }
        if (spell.id === "a.e" && hero.health > 80) { // Swap doesn't work as an attack
            return false;
        }
        if (spell.id === "b.w" && hero.health > 50) { // Swap doesn't work as an attack
            return false;
        }
        if (spell.id === "b.e" && hero.health > 80) { // Swap doesn't work as an attack
            return false;
        }
        return true;
    } else {
...
function focus(hero, opponent) { // When using a spell (e.g. Acolyte Beam, Spirit Missile) - ensure we are focusing the enemy, otherwise we will miss
    if (hero.casting) {
        if (hero.casting.spellId === "blast" || hero.casting.spellId === "retractor" || hero.casting.spellId === "rocket") {
            ...
            return {
                spellId: "retarget",
                target: vectorPlus(hero.pos, vectorFromAngle(hero.heading + Math.PI / 2))
            };
